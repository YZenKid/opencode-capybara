...
8. UX/visual/reference/motion direction missing? `@designer` first. For source-approved 1:1 visual work, route with exact layout/component/token anatomy expectation rather than inspiration-only restyling.
...
## 0. Pre-flight Skill & MCP Discovery (mandatory before the first substantial answer)
...
- `/init-harness` (single entrypoint, per `commands/init-harness.md`) default behavior should detect stack/tooling and conservatively create or update `.opencode/docs/PROJECT_STACK.md`, `.opencode/docs/PROJECT_COMMANDS.md`, `.opencode/docs/FRAMEWORK_PLAYBOOK.md`, and `.opencode/docs/PROJECT_DETECTED_TOOLS.md`.
...
- Inspect the target project's `DESIGN.md` first, then `design-system/DESIGN.md` or equivalent; suggest `/init-harness` (single entrypoint for harness + design init per `commands/init-harness.md`) if substantial UI direction is missing so consolidated harness/design initialization can create or update project guidance. Do not redirect to any separate design-init command.
...
- For portfolio/reference/template work with hero art, portraits, project cards, thumbnails, testimonial/avatar clusters, blog cards, icon badges, or rich backgrounds, assume image-heavy. Use the configured `visual-asset-generator` or another available image-generation workflow for style-equivalent fallback assets unless the designer explicitly records `use-provided-assets`, `licensed-existing-assets`, `no-generation-needed`, or user-directed direct reuse with source/permission notes. Save generated assets in the project’s asset location and disclose source inventory, permission/license status when known, and risk in the final summary.
