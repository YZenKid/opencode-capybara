...
- Run anti-AI-slop preflight before returning: hero fit, nav single-line, CTA contrast/wrap/duplicate intent, eyebrow restraint, layout repetition, image strategy, motion motivation, reduced-motion, professionalism/trust anchors, and no AI-brochure copy.
...
- User-facing Copy Gate: block debug/internal copy, server labels, port numbers, framework jargon, implementation notes, placeholder/trust-breaking copy (`akan diperbarui segera`, `foto stok ilustrasi`, `dokumentasi asli menyusul`), and generic AI-brochure slogans (`pasti bisa`, vague mission filler) in user-facing UI unless explicitly approved.
- Professionalism / Trust Gate: for organization/community/craft sites, require visible trust anchors — real contact readiness, address/location/operating context, legal or organizational identity, and no contradiction between CTAs and readiness copy.
...
- Hero Composition Gate: substantial hero sections need meaningful product/domain composition and asset/image decision, not abstract blobs, floating cards, CSS glass panels, or a lone decorative close-up where process/people imagery is expected.
- Motion Presence Gate: substantial crafted UI must specify motion purpose, hover/focus feedback, and at least subtle aliveness where appropriate; completely static polished-claim UI without rationale should return `needs-polish`.
- Professional Section Anatomy Gate: homepage/primary surface for org/community/craft work must include more than hero — at minimum one credibility/proof section, one product/process preview, and real footer/contact context unless scope explicitly excludes them.
