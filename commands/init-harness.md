...
- Source rules (imported from other agent tools, audit trail): `.opencode/docs/SOURCE_RULES.md` when present — generated by `scripts/rules-harmonizer.py`
- Stack-aware resource suggestions: run `python3 ~/.config/opencode/scripts/stack-resource-suggester.py .` (or read `.opencode/evidence/<task>/stack-resources-suggestion.md` when written)
...
- Harness Preflight Gate: before non-trivial work, `@orchestrator` must verify the target project has a current root `AGENTS.md`, canonical `.opencode/docs/`, and root `DESIGN.md` when UI/design work is involved. Additionally, when the target has existing agent-rules files from other tools, run `python3 ~/.config/opencode/scripts/rules-source-scanner.py .` and `python3 ~/.config/opencode/scripts/rules-harmonizer.py . --apply` to capture the audit trail at `.opencode/docs/SOURCE_RULES.md`; when working in any stack covered by `scripts/data/stack_resources.json`, run `python3 ~/.config/opencode/scripts/stack-resource-suggester.py . --write-evidence .opencode/evidence/<task>/stack-suggestion.md` and confirm the idiomatic patterns with the user before implementation.
...
- Governance script location: canonical governance scripts (e.g. `task-progress.py`, `plan-execution-readiness.py`) are resolved from `${OPENCODE_CONFIG_DIR:-$HOME/.config/opencode}/scripts/`. Never assume the target project ships a local `./scripts/` directory, and never resolve canonical scripts from one. Direct script runners may keep a sibling `scripts/` fallback when the canonical root is missing only as a last resort.
...
- If the target project contains rules files from other agent tools (Claude Code `CLAUDE.md`, Codex `AGENTS.md`/`.codex/`, Cursor `.cursorrules`/`.cursor/rules/`, Windsurf `.windsurfrules`, Continue, Cline, Cody, Roo, Aider, GitHub Copilot, GitLab Duo), detect and harmonize them via:
  - `python3 ~/.config/opencode/scripts/rules-source-scanner.py .` (default scan, `--summary-only` for human output, `--json` for machine output)
  - `python3 ~/.config/opencode/scripts/rules-harmonizer.py .` to write the canonical audit trail at `.opencode/docs/SOURCE_RULES.md`
  - Add `--apply` to append a `## Source Rules` section to the root `AGENTS.md` (idempotent — marker-guarded).
  - Add `--forward-to claude,codex,cursor,windsurf` to write mirror files so external tools stay in sync with OpenCode-native rules. Idempotent (re-running does not duplicate).
- When in doubt, OpenCode-native rules (`AGENTS.md` + `.opencode/docs/`) win over imported external rules. The audit trail records provenance.
- If conflicts arise between imported and OpenCode-native rules, surface them to the user before silently overriding either side.

## Stack-Aware Resource Discovery
- After detecting the project stack, suggest idiomatic skills, MCP servers, and best practices before any non-trivial implementation:
  - `python3 ~/.config/opencode/scripts/stack-resource-suggester.py .` — soft recommender, never auto-installs.
  - Add `--write-evidence <path>` to capture the suggestion as a markdown artifact (e.g. `.opencode/evidence/<task>/stack-resources-suggestion.md`).
  - Use `--list-stacks` to see currently-supported stacks; use `--show <id>` to inspect a single stack.
- Use the registry at `scripts/data/stack_resources.json` as the curated source. Idiomatic patterns in the registry (e.g. "use `php artisan make:*` for all scaffolding") override ad-hoc habits.
- Before writing code in any unfamiliar stack, route to `@librarian` for the official docs URL listed in the suggestion, then re-anchor against the user-confirmed stack version. Do not invent framework APIs from memory.
- 14 stacks currently covered: Laravel, Next.js, React Native/Expo, Flutter, Supabase, Rails, Go, Python/FastAPI, Django, Vue/Nuxt, Svelte/SvelteKit, Tailwind+shadcn, Rust, Postgres. Adding new stacks is data-driven (extend the registry; no code change required).
