...
16. Use the unified helper lane names directly (`@architect`, `@quality-gate`) so a future operator can route quickly without reading every agent file.
...
20. Explicitly state which agent should be used first for default execution (`@orchestrator`).
