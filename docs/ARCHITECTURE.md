...
- **Canonical harness docs** — `.opencode/docs/`
- **Reference-only mirror docs** — `docs/`
...
- `docs/` is reference-only material kept for historical context and future `init-harness` generation.
- `scripts/*` are the enforceable controls.
- `.opencode/` contains canonical harness docs, task-local plans, and evidence.
