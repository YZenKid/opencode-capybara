...
| artifact-planner | primary | 9router/high | opencode-artifact-planner, opencode-system-analyst, opencode-project-manager, opencode-plan-reviewer, opencode-plan-validator, opencode-council, opencode-skill-improver | Artifact-writing SDD/TDD planner using the standalone opencode-capybara plan flow without entering built-in read-only Plan Mode. |
| designer | subagent | 9router/high | opencode-designer, opencode-design-system-engineer, opencode-visual-asset-generator | UI/UX implementation and review lane for polished visuals, motion direction/reduced-motion review, accessibility, visual parity, and visual polish |
...
| fixer | subagent | 9router/low | opencode-fixer, opencode-backend, opencode-frontend, opencode-mobile, opencode-fullstack, opencode-devops | Bounded implementation and testing specialist for Red/Green/Refactor work |
...
| visual-context-extractor | subagent | 9router/medium | opencode-visual-context-extractor | Read-only extraction of observable visual context (layout, text, components, color, state, errors, flows) from images, screenshots, diagrams, and other visual files. Returns structured JSON for caller agents. Does not generate images, give design direction, or edit source. |
