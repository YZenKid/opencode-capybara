...
- `BLOCKED`: required evidence absent and reviewer cannot assess claimed UI/design outcome; **screenshot files** (not just HTML diffs), access, design-system, or reference evidence missing for strong readiness/parity claim.
...
- `PASS_WITH_RISKS`: implementation appears acceptable but evidence has non-blocking gaps or residual visual risk. Missing screenshots for substantial UI is a blocker, not a risk.
...
- AI-slop conformance: for substantial UI, flag `NEEDS_FIX` when any of these mechanical failures are present in shipped UI: card spam / repeated grid anatomy, fake metrics or arbitrary KPIs, debug/internal copy or port numbers in UI, placeholder imagery or blank image frames, centered gradient hero without product/domain composition, generic "modern clean" without source-backed specifics, abstract blobs or CSS glass panels as hero, lorem text, missing reduced-motion support, missing state coverage (empty/loading/error/success), `foto menyusul` placeholder text, decorative symbolic stats without meaningful data, or illustration/pattern-card hero when reference/domain requires real photography. **Slop-containing UI must not receive `PASS` or `PASS_WITH_RISKS`.**
- Reference feel parity conformance: for reference-driven/community/craft/food/agriculture/artisan/organization work, verify the implementation captures reference essence (warmth, humanity, texture, domain-specific content, lived reality) not just structure. If UI feels sterile/template-built despite structural similarity, return `NEEDS_FIX`.
- Domain texture conformance: hero/product/community sections must show real photography or generated domain-specific imagery when reference/domain requires it. Physical objects, natural materials, hands working, environment, or local context should be present when those are the source of trust/warmth in the reference. Missing domain texture is `NEEDS_FIX`.
- Cross-lane experiential evidence conformance: for substantial UI/reference work, run `python3 ~/.config/opencode/scripts/verify-visual-quality-evidence.py --project-root . --task <task-id>`. Missing `.opencode/evidence/<task-id>/visual-quality-contract.md`, `.opencode/evidence/<task-id>/visual-rubric.md`, `.opencode/evidence/<task-id>/reference-essence.md`, required per-surface `reject_if` in the plan, OR actual screenshot files (PNG/WebP) in the evidence folder is `NEEDS_FIX` or `BLOCKED` depending on severity.
- Professionalism / trust conformance: for org/community/craft sites, flag `NEEDS_FIX` when homepage ships contradictory contact readiness copy, lacks address/location/operating context or legal/org identity, lacks proof/credibility section, or uses AI-brochure filler slogans instead of concrete trust-building content.
- Motion/aliveness conformance: for substantial crafted UI, flag `NEEDS_FIX` when there is no meaningful hover/focus/scroll/texture motion and no explicit rationale for intentionally static treatment; reduced-motion support still required.
