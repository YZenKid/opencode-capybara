...
- Cross-lane experiential evidence conformance: for substantial UI/reference work, run `python3 ~/.config/opencode/scripts/verify-visual-quality-evidence.py --project-root . --task <task-id>`. Missing `.opencode/evidence/<task-id>/visual-quality-contract.md`, `.opencode/evidence/<task-id>/visual-rubric.md`, `.opencode/evidence/<task-id>/reference-essence.md`, or required per-surface `reject_if` in the plan is `NEEDS_FIX` or `BLOCKED` depending on severity.
