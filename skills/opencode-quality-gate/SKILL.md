...
- For full-revamp or flow-redesign claims, require `python3 ~/.config/opencode/scripts/design-revamp-contract.py --project-root . --task-id <id> --class <polish|surface-refresh|flow-redesign|full-revamp>` and verify all required artifacts in `.opencode/evidence/<task-id>/`.
- For new shared UI components introduced in implementation, require `python3 ~/.config/opencode/scripts/catalog-reuse-enforcer.py --project-root . --files <changed-files>` or equivalent proof the component was added to `.opencode/design-system/catalog.json`.
