...
export function createServer(deps = {}) {
  ...
  // ponytail: fixed web models; add explicit routing only when per-call selection is needed again.
  const webSearchModel = 'search'
  const webFetchModel = 'fetch'
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
      ...
      { name: 'web_search', description: 'Web search via 9Router. Accepts query and optional max_results only.', inputSchema: { type: 'object', additionalProperties: false, required: ['query'], properties: { query: { type: 'string' }, max_results: { type: 'number', default: 5 } } } },
      { name: 'web_fetch', description: 'Fetch URL via 9Router. Accepts URL only.', inputSchema: { type: 'object', additionalProperties: false, required: ['url'], properties: { url: { type: 'string' } } } },
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      ...
      if (name === 'web_search') return { content: toolText(await requestJsonImpl('/v1/search', { model: webSearchModel, query: args.query, search_type: 'web', max_results: args.max_results ?? 5 })) }
      if (name === 'web_fetch') return { content: toolText(await requestJsonImpl('/v1/web/fetch', { model: webFetchModel, url: args.url, format: 'markdown' })) }
