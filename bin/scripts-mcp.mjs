...
export const TOOL_REGISTRY = Object.freeze([
  ...
  { name: 'scripts_memory_query', sourceScript: 'project-memory.py', classification: 'memory-read', laneRecommendation: '@orchestrator', schema: withRoot({ context: z.string().min(1), limit: z.number().int().positive().optional() }), inputSchema: jsonSchema({ context: { type: 'string' }, limit: { type: 'integer', minimum: 1 } }, ['context']), cliFallback: 'python3 ~/.config/opencode/scripts/project-memory.py --load --context <context> [--limit <limit>]' },
  ...
  { name: 'scripts_memory_reuse_check', sourceScript: 'project-memory.py', classification: 'memory-read', laneRecommendation: '@orchestrator', schema: withRoot({}), inputSchema: jsonSchema({}, []), cliFallback: 'python3 ~/.config/opencode/scripts/project-memory.py --list-proposals' },
...
function fixedArgs(name, args, root) {
  ...
  switch (name) {
    ...
    case 'scripts_memory_query': return ['project-memory.py', '--load', '--context', args.context, '--limit', String(args.limit || 10)]
    ...
    case 'scripts_memory_reuse_check': return ['project-memory.py', '--list-proposals']
