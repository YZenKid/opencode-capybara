...
> Vibe: Calm productivity. Soft neutrals, low-saturation accents, generous whitespace, reduced motion.

> **Source**: https://open-design.ai/plugins/systems/example-calm
> **License**: Apache-2.0
> **Indexed by**: `python3 ~/.config/opencode/scripts/design-source-importer.py --download calm`
> **Status**: downloaded (full content). For live refresh: `--refresh --slug calm`.

---

## 1. Visual Theme & Atmosphere

Calm's productivity discipline: soft neutrals, low-saturation accents, generous whitespace, reduced motion.

This system commits to calm productivity. It is **not** a generic modern-clean aesthetic; the rules below are concrete and rejectable.

## 2. Color Palette & Roles

### Primary
- **Ink** (`#2d3748`): Primary text, dark surfaces, structural strokes.
- **Accent** (`#718096`): Action color, focus, primary CTA, selected state.
- **Surface** (`#f7fafc`): Light canvas for content blocks; cards, panels, sheets.

### Secondary
- **Muted** (`#a0aec0`): Secondary text, borders, dividers, low-emphasis fills.
- **Subtle** (`#edf2f7`): Hover, disabled, inactive.

### Semantic
- **Success** (`#68d391`): Confirmations, positive deltas.
- **Warning** (`#f6ad55`): Caution, recoverable errors.
- **Danger** (`#fc8181`): Destructive actions, errors, critical alerts.
- **Info** (`#63b3ed`): Tips, non-blocking notices.

### Dark variants (when applicable)
- **Dark ground** (`#1a202c`): For optional surfaces.
- **Dark ink** (`#f7fafc`): Primary text on dark ground.
...
| Mono | Fira Code | 14/20 | 400 | 1.5 | 0 |
...
4. Match the system's vibe: calm productivity means low-saturation, generous space.
...
- **Catalog source**: https://open-design.ai/plugins/systems/example-calm
- **License**: Apache-2.0
...
> Only present in forks via `python3 ~/.config/opencode/scripts/design-system-fork.py --base calm`.
