...
> Vibe: Premium SaaS marketing. Soft gradients, mesh background, generous whitespace, violet accent.

> **Source**: https://open-design.ai/plugins/systems/example-stripe
> **License**: Apache-2.0
> **Indexed by**: `python3 ~/.config/opencode/scripts/design-source-importer.py --download stripe`
> **Status**: downloaded (full content). For live refresh: `--refresh --slug stripe`.

---

## 1. Visual Theme & Atmosphere

Stripe's premium-marketing discipline: gradient mesh as atmosphere, not as hero subject; product remains the hero.

This system commits to premium SaaS marketing. It is **not** a generic modern-clean aesthetic; the rules below are concrete and rejectable.

## 2. Color Palette & Roles

### Primary
- **Ink** (`#0a2540`): Primary text, dark surfaces, structural strokes.
- **Accent** (`#635bff`): Action color, focus, primary CTA, selected state.
- **Surface** (`#ffffff`): Light canvas for content blocks; cards, panels, sheets.

### Secondary
- **Muted** (`#425466`): Secondary text, borders, dividers, low-emphasis fills.
- **Subtle** (`#f6f9fc`): Hover, disabled, inactive.

### Semantic
- **Success** (`#0d7c5f`): Confirmations, positive deltas.
- **Warning** (`#b88500`): Caution, recoverable errors.
- **Danger** (`#dc2626`): Destructive actions, errors, critical alerts.
- **Info** (`#3b82f6`): Tips, non-blocking notices.

### Dark variants (when applicable)
- **Dark ground** (`#0a2540`): For hero/marketing surfaces.
...
|---|---|---|---|---|---|
| Display | Sohne / Inter Display | 56/64 | 700 | 1.05 | -0.02em |
...
| Mono | Sohne Mono | 14/20 | 400 | 1.5 | 0 |
...
4. Match the system's vibe: premium SaaS marketing means clear value, restrained drama.
...
- **Photo style**: lifestyle + product hybrid, optimistic
- **Illustration style**: gradient mesh, abstract
...
- **Catalog source**: https://open-design.ai/plugins/systems/example-stripe
- **License**: Apache-2.0
...
> Only present in forks via `python3 ~/.config/opencode/scripts/design-system-fork.py --base stripe`.
