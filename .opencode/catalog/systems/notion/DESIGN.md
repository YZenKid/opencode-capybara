...
> Vibe: Notion. Warm neutral, low-contrast UI, friendly serif accents, soft borders.

> **Source**: https://open-design.ai/plugins/systems/example-notion
> **License**: Apache-2.0
> **Indexed by**: `python3 ~/.config/opencode/scripts/design-source-importer.py --download notion`
> **Status**: downloaded (full content). For live refresh: `--refresh --slug notion`.

---

## 1. Visual Theme & Atmosphere

Notion's friendly discipline: warm neutrals, soft borders, low contrast that doesn't strain.

This system commits to warm neutral. It is **not** a generic modern-clean aesthetic; the rules below are concrete and rejectable.

## 2. Color Palette & Roles

### Primary
- **Ink** (`#37352f`): Primary text, dark surfaces, structural strokes.
- **Accent** (`#2383e2`): Action color, focus, primary CTA, selected state.
- **Surface** (`#ffffff`): Light canvas for content blocks; cards, panels, sheets.

### Secondary
- **Muted** (`#9b9a97`): Secondary text, borders, dividers, low-emphasis fills.
- **Subtle** (`#f7f6f3`): Hover, disabled, inactive.

### Semantic
- **Success** (`#0f7b0f`): Confirmations, positive deltas.
- **Warning** (`#cb912f`): Caution, recoverable errors.
- **Danger** (`#e03e3e`): Destructive actions, errors, critical alerts.
- **Info** (`#2383e2`): Tips, non-blocking notices.

### Dark variants (when applicable)
- **Dark ground** (`#191919`): For toggleable surfaces.
...
|---|---|---|---|---|---|
| Display | Lora | 56/64 | 700 | 1.05 | -0.02em |
...
4. Match the system's vibe: warm neutral means low-contrast friendly.
...
- **Catalog source**: https://open-design.ai/plugins/systems/example-notion
- **License**: Apache-2.0
...
> Only present in forks via `python3 ~/.config/opencode/scripts/design-system-fork.py --base notion`.
