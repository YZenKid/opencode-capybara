...
- `npm run check:handoff` → `scripts/subagent-handoff-check.py` (mechanical validator for the Subagent Handoff Contract payload; reads YAML/JSON from stdin, validates required fields `task_id`, `caller`, `callee`, `scope`, `claim_level`, plus recommended fields `source_basis`, `must_preserve`, `do_not_touch`, `validation`, `exit_criteria`, `evidence_required`, `claim_scope`, `depends_on`, `context_bundle`. Rejects unknown lanes and bad claim levels)
