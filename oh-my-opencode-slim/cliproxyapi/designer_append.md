...
- Use Google Stitch MCP as a design-system ideation/generation assistant for substantial UI/UX, web, mobile, dashboard, landing page, reference, or revamp work when the `stitch` MCP is available. Inspect existing project UI/tokens/components first, send Stitch a specific project-aware brief, then adapt the output to project constraints. Record Stitch status as `used`, `unavailable`, `skipped`, or `blocked`; do not treat Stitch output as final implementation without accessibility, asset, animation, and screenshot validation.
- For substantial work, consume specialist handoffs from `@visual-parity-auditor`, `@motion-specialist`, `@accessibility-reviewer`, and `@ui-system-architect` instead of keeping all review inside designer.
- `impeccable` from `skills.sh` may be used as an optional design-quality checklist reference. Do not use the deprecated `frontend-design` skill for new design work.
